...
import {
  Plus, Search, ArrowDownLeft, ArrowUpRight, TrendingUp, TrendingDown,
  X, ChevronLeft, ChevronRight, MoreHorizontal, Pencil, Trash2,
  Wallet, Tag, FileText, Calendar,
} from 'lucide-react'
...
export function TransactionsPage() {
  ...
  const refreshStats = useCallback(() => {
    getTransactions({ limit: 9999 }).then(({ data, meta }) => {
  ...
  const openAdd = () => { setEditingTx(null); setForm(emptyForm); setIsOpen(true) }
  ...
  const closeModal = () => { setIsOpen(false); setEditingTx(null); setError(null) }
  const handleFilterChange = (f: typeof filter) => { setFilter(f); setPage(1) }

  const handleSubmit = async () => {
    if (!form.description.trim() || !form.amount || !form.categoryId || !form.accountId || !form.date) {
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        {[
          { label: 'Total Transaksi', value: `${allTotal} transaksi`, up: undefined as boolean | undefined },
          { label: 'Total Pemasukan', value: formatCurrency(totalIncome), up: true },
          { label: 'Total Pengeluaran', value: formatCurrency(totalExpense), up: false },
        ].map((s, i) => (
          <div key={i} className="bg-white dark:bg-[#1a1f2e] rounded-2xl px-5 py-4 flex items-center justify-between shadow-sm border border-brand-stroke/5 dark:border-white/5">
            <div>
              <p className="text-[10px] font-bold text-brand-stroke/40 dark:text-slate-500 uppercase tracking-widest">{s.label}</p>
              <p className={`text-lg font-extrabold mt-0.5 tracking-tight ${
                s.up === true ? 'text-emerald-600' : s.up === false ? 'text-brand-tertiary' : 'text-brand-stroke'
              }`}>{s.value}</p>
            </div>
            {s.up === true && <TrendingUp className="w-5 h-5 text-emerald-500 opacity-60" />}
            {s.up === false && <TrendingDown className="w-5 h-5 text-brand-tertiary opacity-60" />}
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            onClick={openAdd}
            className="flex items-center gap-2 px-4 sm:px-5 py-2.5 bg-brand-bg text-brand-headline text-sm font-bold rounded-xl shadow-md hover:opacity-90 active:scale-95 transition-all duration-200 shrink-0"
          >
      ...
      <div className="bg-white dark:bg-[#1a1f2e] rounded-3xl shadow-sm border border-brand-stroke/5 dark:border-white/5 overflow-hidden">
        <div className="px-6 py-5 border-b border-brand-stroke/5 dark:border-white/5">
          <h3 className="text-base font-bold text-brand-stroke dark:text-white">Daftar Transaksi</h3>
          <p className="text-xs text-brand-stroke/40 dark:text-slate-400 font-medium mt-0.5">{totalCount} transaksi ditemukan</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[600px] text-left border-collapse">
            ...
            <tbody className="divide-y divide-brand-stroke/5 dark:divide-white/5">
              {loading && (
                <tr>
                  <td colSpan={6} className="px-6 py-16 text-center text-sm text-brand-stroke/40 dark:text-slate-500 font-medium">
                    Memuat...
                  </td>
                </tr>
              )}
              ...
              {!loading && transactions.map((tx) => (
                <tr key={tx.id} className="group hover:bg-slate-50/60 dark:hover:bg-white/5 transition-colors duration-150">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${
                        tx.type === 'INCOME' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-brand-tertiary'
                      }`}>
                        {tx.type === 'INCOME'
                  ...
                  <td className={`px-3 py-4 text-right text-sm font-extrabold ${
                    tx.type === 'INCOME' ? 'text-emerald-600' : 'text-brand-tertiary'
                  }`}>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center px-4 pt-4 ">
          ...
          {/* Modal Card */}
          <div className="relative w-full max-w-md bg-white dark:bg-[#1a1f2e] rounded-3xl shadow-2xl flex flex-col" style={{ maxHeight: 'calc(100vh - 120px)' }}>
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-white/5 shrink-0">
              <div>
                ...
                <p className="text-xs text-brand-stroke/40 dark:text-slate-400 font-medium mt-0.5">
                  {editingTx ? 'Ubah detail transaksi' : 'Catat pemasukan atau pengeluaran baru'}
                </p>
              </div>
              <button
                onClick={closeModal}
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar px-6 py-5 space-y-5">
              ...
              {/* Type Toggle */}
              <div>
                <label className="text-[10px] font-bold text-brand-stroke/40 dark:text-slate-500 uppercase tracking-widest block mb-2">Jenis Transaksi</label>
              ...
              {/* Nominal */}
              <div>
                <label className="text-[10px] font-bold text-brand-stroke/40 dark:text-slate-500 uppercase tracking-widest block mb-2">Nominal</label>
              ...
              <div>
                <label className="text-[10px] font-bold text-brand-stroke/40 dark:text-slate-500 uppercase tracking-widest block mb-2">
                  <FileText className="w-3 h-3 inline mr-1" />Keterangan
                </label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-[10px] font-bold text-brand-stroke/40 dark:text-slate-500 uppercase tracking-widest block mb-2">
                  <Wallet className="w-3 h-3 inline mr-1" />Rekening
                </label>
                <select
