...
import { periodRange } from '../../features/period/payPeriod'
import {
...
export function ExpensesPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getPeriodTransactions({ type: 'EXPENSE', month, year }),
  ...
  const range = periodRange(year, month, currentPeriod.payday ?? 1)
  const daysInPeriod = (Date.parse(range.end) - Date.parse(range.start)) / 86_400_000
  const dailyAverage = daysInPeriod > 0 ? Math.round(totalExpense / daysInPeriod) : 0
        ...
        <div className="bg-white dark:bg-[#1a1f2e] rounded-3xl p-6 shadow-sm border border-brand-stroke/5 dark:border-white/5 lg:col-span-2 flex flex-col">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
            <div>
              ...
              <p className="text-xs text-brand-stroke/40 dark:text-slate-400 font-medium mt-0.5">Pantau penggunaan anggaran periode gaji</p>
            </div>
              ...
              {!loading && filteredTransactions.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-sm text-brand-stroke/40 dark:text-slate-500 font-medium">
                    {searchTerm ? 'Tidak ada hasil pencarian' : 'Belum ada transaksi pengeluaran periode ini'}
                  </td>
                </tr>
              )}
          ...
          >
            <div className="px-6 py-5 border-b border-brand-stroke/5 dark:border-white/5 shrink-0">
              ...
              >
                ...
                {isCopyingBudget ? 'Menyalin...' : 'Salin dari periode lalu'}
              </button>
            </div>
